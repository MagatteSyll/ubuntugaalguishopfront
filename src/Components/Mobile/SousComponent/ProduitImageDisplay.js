...
setimage({...image,image:image,size:size,color:color,quantite:qte,id:lid,
  ...
  return (
    ...
    <Image className='imgdetailmobile' src={`https://gaalguishopbackend.herokuapp.com${image.image}`} />
   
    <div className='mt-2 container'>
    <IonSegment className='detailsegment'>
        {produitimage.map(pi=>
        <button className='btndrop btndetail' onClick={()=>handleclick(pi.id)}>
        <Image className='imgbtnmobiledetail' src={`https://gaalguishopbackend.herokuapp.com${pi.image}`} />
        </button>)}
    </IonSegment>
    </div>
    </IonCol>
